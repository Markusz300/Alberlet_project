          ...
          <q-item v-for="albi in pendingAlbis" :key="albi.id" class="q-py-md">
            ...
            <q-item-section side>
              <div class="row q-gutter-xs">
                <q-btn icon="check" color="green" round unelevated size="sm" @click="setStatus(albi.id, 1)"><q-tooltip>Jóváhagyás</q-tooltip></q-btn>
                <q-btn icon="edit" color="blue" round unelevated size="sm" @click="goToEdit(albi.id)"><q-tooltip>Szerkesztés</q-tooltip></q-btn>
                <q-btn icon="delete" color="red" round unelevated size="sm" @click="deleteAlbi(albi.id)"><q-tooltip>Törlés</q-tooltip></q-btn>
              </div>
            </q-item-section>
          </q-item>
          <div v-if="!pendingAlbis.length" class="text-center q-pa-xl text-grey-6">Nincs jóváhagyásra váró hirdetés.</div>
        </q-list>
      </q-tab-panel>
        ...
        <q-list bordered separator class="bg-white rounded-borders shadow-1">
          <q-item v-for="albi in activeAlbis" :key="albi.id" class="q-py-md">
            ...
            <q-item-section side>
              <div class="row q-gutter-xs">
                <q-btn icon="edit" color="blue" round unelevated size="sm" @click="goToEdit(albi.id)"><q-tooltip>Szerkesztés</q-tooltip></q-btn>
                <q-btn icon="block" color="orange" round unelevated size="sm" @click="setStatus(albi.id, 0)"><q-tooltip>Hirdetés elrejtése</q-tooltip></q-btn>
                <q-btn icon="delete" color="red" round unelevated size="sm" @click="deleteAlbi(albi.id)"><q-tooltip>Törlés</q-tooltip></q-btn>
              </div>
            </q-item-section>
          </q-item>
          <div v-if="!activeAlbis.length" class="text-center q-pa-xl text-grey-6">Nincs aktív hirdetés a rendszerben.</div>
        </q-list>
      </q-tab-panel>
    </q-tab-panels>
  </q-page>
